...
@pytest.fixture(scope="session")
def contract_name(StrategyCurvesBTCFactoryClonable):
    contract_name = StrategyCurvesBTCFactoryClonable
    yield contract_name
...
@pytest.fixture(scope="session")
def no_profit():
    no_profit = False  # we do outrun the profit sometimes, though
    yield no_profit
...
def sleep_time():
    ...
    hours_to_sleep = (
        72  # 72 for pBTC, but we outrun some of the rewards testing in curve_and_convex
    )
